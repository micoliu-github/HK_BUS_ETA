...
const BASE_URL = 'https://data.etabus.gov.hk/v1/transport/lwb';

export async function getLWBRoutes(routeNumber: string): Promise<Route[]> {
  try {
    ...
    if (response.ok) {
      ...
      if (data.data && data.data.length > 0) {
        return data.data.map((r: any) => ({ ...r, company: 'LWB' })) as Route[];
      }
    }

    // Fallback: Fetch all routes and filter
    console.warn(`LWB Route ${upperRoute} not found via direct endpoint, fetching all routes...`);
    ...
    console.error('Error fetching LWB route:', error);
    return [];
  }
}

export async function getLWBRouteStops(route: string, direction: string, serviceType: string) {
  try {
    ...
    console.error('Error fetching LWB route stops:', error);
    return [];
  }
}

export async function getLWBRouteETA(route: string, serviceType: string) {
  try {
    ...
    console.error('Error fetching LWB route ETA:', error);
    return [];
  }
}

export async function getLWBStopETA(stopId: string, route: string, serviceType: string) {
  try {
    ...
    console.error('Error fetching LWB ETA:', error);
    return [];
  }
}

export async function getAllLWBStopETA(stopId: string) {
  try {
    ...
    console.error('Error fetching all LWB ETAs for stop:', error);
    return [];
  }
}
